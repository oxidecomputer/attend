...
/// The returned instant marks the beginning of the silence.
#[test]
fn speech_then_silence_triggers_split() {
    let silence_dur = Duration::from_millis(500);
    ...
    let silence_instant = now + Duration::from_millis(500);
    let silence = silence_signal(11200);
    let silence_chunk = make_chunk(silence, silence_instant);
    ...
    assert_eq!(
        split.unwrap(),
        silence_instant,
        "split instant should mark silence start"
    );
...
fn continuous_speech_no_split() {
    let mut det = SilenceDetector::new(VAD_RATE, Duration::from_millis(500));
    let now = Instant::now();

    // Feed 2 seconds of speech in 100ms chunks.
    for i in 0..20 {
        let chunk_instant = now + Duration::from_millis(i * 100);
        let speech = speech_signal(1600); // 100ms at 16 kHz
        let chunk = make_chunk(speech, chunk_instant);
        assert!(
...
fn short_silence_no_split() {
    ...
    let t2 = now + Duration::from_millis(600);
